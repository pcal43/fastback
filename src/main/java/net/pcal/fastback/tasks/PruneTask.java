...
public class PruneTask implements Runnable {
    ...
    public void run() {
        ...
        for (final SnapshotId sid : toPrune) {
            log.hud(localized("fastback.hud.prune-pruning", sid.getName()));
            try {
        ...
        log.hud(localized("fastback.hud.prune-done", pruned));
        if (pruned > 0) {
            log.hud(localized("fastback.hud.prune-suggest-gc"));
        }
    }
}
